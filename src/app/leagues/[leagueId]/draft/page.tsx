...
export default async function DraftPage({
  ...
  if (!user) {
    return (
      <LeaguePageShell
        ...
        pageTitle="League draft"
        pageSubtitle="Sign in to view this league draft."
      >
  ...
  if (!membership) {
    return (
      <LeaguePageShell
        ...
        seasonLabel={`Season ${league.season.name} ${league.season.year}`}
        pageTitle="League draft"
        pageSubtitle="You need to join this league before viewing its draft."
      >
        <Link
          href="/leagues"
          className="text-sm font-medium text-[var(--text-muted)] underline-offset-4 transition hover:text-[var(--text)] hover:underline"
  ...
  if (!league.season.isActive) {
    return (
      <LeaguePageShell
        ...
        seasonLabel={`Season ${league.season.name} ${league.season.year}`}
        pageTitle="League draft"
        pageSubtitle="Make picks, manage your queue, and track the draft board."
  ...
  const boardRounds = Array.from({ length: rounds }, (_, index) => {
    const roundNumber = index + 1;
    const slots = teams.map((team) => {
      const pick = picksByRoundTeam.get(`${roundNumber}-${team.id}`);
      const isCurrentPick =
      ...
      return {
        ...
        clubLabel: pick?.player.club
          ? getClubDisplayName(pick.player.club.slug, pick.player.club.name)
          : null,
        isCurrentPick,
        isMyTeam,
      };
    });
    return { round: roundNumber, slots };
  });
  ...
  return (
    ...
    >
      ...
      <div className="hidden sm:block">
        {!draft ? (
          <EmptyState
            title="Draft not created yet"
            description="The commissioner will create the draft when the league is ready."
          />
        ) : (
